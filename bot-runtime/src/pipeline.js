...
소스 문서에 포함된 모든 항목을 빠짐없이 슬라이드화한다.

금지 행동:
- 요약 금지
- 병합 금지 (유사해 보여도 별도 슬라이드)
- 생략 금지 ("중요도 낮음" 판단 금지)
- "간략히" "대표적으로" 같은 표현 금지

의무 행동:
- 소스의 모든 수치, 데이터, 시사점 → 각각 슬라이드 1장 이상
- 작업 전: 소스에서 항목 수 직접 카운트해서 말할 것
  예) "소스 확인: 총 23개 데이터 포인트 발견, 슬라이드 23장 이상 생성합니다"
- 작업 후: "소스 N개 → 슬라이드 N장 반영 완료" 확인 출력
- 빠진 항목 있으면 스스로 감지하고 추가할 것
