...
## ⚠️ 다양성 강제 (반드시 준수 — 실수 시 재요청 사유)
3개 옵션이 **서로 확실히 달라야** 함. 다음 축으로 겹치지 마라:

1. **배경 톤 분포**: 3개 중 **최소 1개는 화이트/밝은 계열** (배경이 흰색·밝은 그레이·연한 베이지 등).
   모든 옵션이 다크 계열이면 실패. 아래처럼 배분하라:
   - 예시 A: ① 다크 보이드  / ② 화이트 미니멀 / ③ 중간 웜 그레이
   - 예시 B: ① 다크 네이비 + 골드 / ② 퓨어 화이트 + 에메랄드 / ③ 베이지 + 다크 브라운
   - 예시 C: ① 해킹룸 다크 / ② 신제품 발표장 화이트 / ③ 선라이즈 오렌지

2. **레이아웃 축**: 카드 그리드 / 전면 타이포 / 매거진 컬럼 / 대시보드 등 **서로 다른 구조**.

3. **인터랙션 축**: 카운터업 / fade-in 순차 / hover 변형 / 슬라이드 전환 효과 등 **서로 다른 동작**.

4. **레퍼런스 축**: Stripe / Apple / Linear / WIRED / Spotify 등 **서로 다른 레퍼런스** 에서 영감.

5. **오디언스 맞춤**: 최소 1개는 **안전·신뢰 톤** (경영진 대상), 최소 1개는 **실용·모던 톤** (실무진 대상), 최소 1개는 **임팩트 톤** (한 장 한 메시지).

**검증**: 3개 options 중 colors·layout·interaction 필드를 나란히 놓았을 때 **한눈에 차이가 보여야** 함.
colors 필드가 전부 "다크 ○○" 면 실패 → 다시 써라.`;
